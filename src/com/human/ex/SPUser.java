...
public class SPUser {
	...
	private void shortPhrase() {
		...
		for(int i=0;i<userInput[0].length();i++) {
			userInput2[0][i]=userInput[0].split("")[i];
			if(userInput2[0][i]!=null&&shortP2[0][i]!=null) {
				if(shortP2[0][i].equals(userInput2[0][i])) {
					score++;
				}
			}
		}
		for(int i=0;i<userInput[1].length();i++) {
			userInput2[1][i]=userInput[1].split("")[i];
			if(userInput2[1][i]!=null&&shortP2[1][i]!=null) {
				if(shortP2[1][i].equals(userInput2[1][i])) {
					score++;
				}
			}
		}
		for(int i=0;i<userInput[2].length();i++) {
			userInput2[2][i]=userInput[2].split("")[i];
			if(userInput2[2][i]!=null&&shortP2[2][i]!=null) {
				if(shortP2[2][i].equals(userInput2[2][i])) {
					score++;
				}
			}
		}
		for(int i=0;i<userInput[3].length();i++) {
			userInput2[3][i]=userInput[3].split("")[i];
			if(userInput2[3][i]!=null&&shortP2[3][i]!=null) {
				if(shortP2[3][i].equals(userInput2[3][i])) {
					score++;
				}
			}
		}
		for(int i=0;i<userInput[4].length();i++) {
			userInput2[4][i]=userInput[4].split("")[i];
			if(userInput2[4][i]!=null&&shortP2[4][i]!=null) {
				if(shortP2[4][i].equals(userInput2[4][i])) {
					score++;
				}
			}
		}		
